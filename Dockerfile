...
RUN git init -q .  && git remote add origin "https://github.com/${FRONTEND_SLUG}.git"  && git fetch --depth 1 origin "${FRONTEND_REF}"  && git checkout -q FETCH_HEAD
...
ENV PYTHONUNBUFFERED=1     PYTHONDONTWRITEBYTECODE=1     PIP_NO_CACHE_DIR=1     PYTHONPATH=/app/src     MPARLAMENT_STATIC_DIR=/app/static     MPARLAMENT_UPLOAD_DIR=/app/uploads     MPARLAMENT_DATABASE_URL=sqlite+aiosqlite:////app/data/mparlament.db
...
RUN chmod +x /usr/local/bin/entrypoint.sh  && mkdir -p /app/data /app/uploads/resolutions  && useradd --create-home --uid 10001 app  && chown -R app:app /app
USER app
